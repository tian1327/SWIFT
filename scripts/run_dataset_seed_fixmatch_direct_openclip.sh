...
### FixMatch Naive, no stage 1,  no T_loss and T_pseudo-label

# Define arrays of values for each parameter

# prefix="batch_FixMatch-direct_LP-text-cls"
prefix="batch_DebiasPL-direct_LP-text-cls"
                                ...
                                for unlabeled_split in "${unlabeled_splits[@]}"; do
                                    for temp_scheme in "${temp_scheme_list[@]}"; do
                                        for temperature in "${temperature_list[@]}"; do
                                            ...
                                            # classifier learned using LP with text embedding initialization, no temperature learning or text scaling
                                            cls_path="output/LinearProbing_text-init_${model_cfg}_50epochs/output_${dataset}/${dataset}_probing_fewshot_REAL-Prompt_${shots}shots_seed${seed}/stage1_model_best.pth"
