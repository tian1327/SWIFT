...
FONT_SIZE = 13
LEGEND_SIZE = 10
LINE_WIDTH = 1.5  # control overall line thickness
...
def plot_loss_acc(path_list):
    ...
    # === Two rows, one column ===
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(6, 4.5), sharex=False)
    ...
    ax1.plot(epoch, losses[0], linestyle='--', linewidth=LINE_WIDTH,
             label=r'$T_{loss}$ init. to 1.0', color='tab:red')
    ax1.plot(epoch, losses[1], linestyle='--', linewidth=LINE_WIDTH,
             label=r'$T_{loss}$ init. to 0.1', color=blue_shades[0])
    ax1.plot(epoch, losses[2], linestyle='--', linewidth=LINE_WIDTH,
             label=r'$T_{loss}$ init. to 0.07', color=blue_shades[1])
    ax1.plot(epoch, losses[3], linestyle='--', linewidth=LINE_WIDTH,
             label=r'$T_{loss}$ init. to 0.01', color=blue_shades[2])

    ax1.set_xlabel('Training epochs', fontsize=FONT_SIZE)
    ax1.set_ylabel('Training loss', fontsize=FONT_SIZE)
    ax1.tick_params(axis='both', which='major', labelsize=FONT_SIZE)  # ✅ set tick font size
    ax1.grid(alpha=0.2)
    ax1.legend(
        loc='center right',
    ...
    ax2.text(
        epoch[-1]*0.48, 51.0, 'Linear Probing Acc.=53.5%',
    ...
    ax2.tick_params(axis='both', which='major', labelsize=FONT_SIZE)  # ✅ set tick font size
    ax2.grid(alpha=0.2)

    plt.tight_layout()
    plt.savefig('FSFT_temp_loss_acc_all.pdf', dpi=300)
