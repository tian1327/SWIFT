...
def plot_loss_acc(path_list):
    ...
    ax2.plot(epoch, accs[0], linestyle='--', linewidth=LINE_WIDTH,
             label=r'$T_{loss}$ init. 1.0', color=color_map[1.0], alpha=0.6)
    ax2.plot(epoch, accs[1], linestyle='--', linewidth=LINE_WIDTH,
             label=r'$T_{loss}$ init. 0.1', color=color_map[0.1], alpha=0.6)
    ax2.plot(epoch, accs[2], linestyle='--', linewidth=LINE_WIDTH,
             label=r'$T_{loss}$ init. 0.07', color=color_map[0.07], alpha=0.6)
    ax2.plot(epoch, accs[3], linestyle='--', linewidth=LINE_WIDTH,
             label=r'$T_{loss}$ init. 0.01', color=color_map[0.01], alpha=0.6)

    # Fixed (solid)
    ax2.plot(epoch, accs[4], linestyle='-', linewidth=LINE_WIDTH,
             label='No TT ($T_{loss}=1.0$)', color=color_map[1.0], alpha=0.6)
    ax2.plot(epoch, accs[5], linestyle='-', linewidth=LINE_WIDTH,
             label=r'$T_{loss}=0.1$', color=color_map[0.1], alpha=0.6)
    ax2.plot(epoch, accs[6], linestyle='-', linewidth=LINE_WIDTH,
             label=r'$T_{loss}=0.07$', color=color_map[0.07], alpha=0.6)
    ax2.plot(epoch, accs[7], linestyle='-', linewidth=LINE_WIDTH,
             label=r'$T_{loss}=0.01$', color=color_map[0.01], alpha=0.6)
    ...
    plt.savefig('FSFT_temp_loss_acc_all_zoom.png', bbox_inches='tight', dpi=300)
    plt.savefig('FSFT_temp_loss_acc_all_zoom.pdf', bbox_inches='tight', dpi=300)
    print("Saved: FSFT_temp_loss_acc_all_zoom.[png/pdf]")
