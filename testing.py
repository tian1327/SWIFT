...
def extract_confidence(args, model, classifier_head, test_loader):
    ...
    with torch.no_grad():
        ...
        for i, val_data in enumerate(test_loader):
            ...
            # logits_list.append(logit.cpu().numpy()) # dim: batch_size x num_classes
            logits_list.append(pseudo_label.cpu().numpy()) # dim: batch_size x num_classes
            predicted_labels.append(targets_u.cpu().numpy())
    ...
    out_file = f'{args.folder}/{args.model_cfg}_{args.T}_{args.dataset}_{args.shots}_{args.seed}_test_confidence.csv'
    ...
    args.logger.info(f'logits.shape: {logits.shape}')
    args.logger.info(f'predicted_labels.shape: {predicted_labels.shape}')
    args.logger.info(f'ground_truth_labels.shape: {ground_truth_labels.shape}')
    ...
    logits_file = f'{args.folder}/{args.model_cfg}_{args.T}_{args.dataset}_{args.shots}_{args.seed}_test_logits.pkl'
    with open(logits_file, 'wb') as f:
        pickle.dump(logits_info, f)
    args.logger.info(f'Extracted logits saved to: {logits_file}')
